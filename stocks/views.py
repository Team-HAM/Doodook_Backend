...
from rest_framework.response import Response
from rest_framework import status
from doodook.redis_queue import enqueue_stock_code
from django.core.cache import cache
from datetime import datetime, timedelta

class StockPriceChangeView(APIView):
    permission_classes = []           # AllowAny 생략해도 기본 허용
    ...
    def get(self, request):
        ...
        if not (stock_code.isdigit() and len(stock_code) == 6):
            return Response(
                {"status": "error", "message": "유효하지 않은 주식 코드입니다."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        # 3) 이미 계산된 결과가 있으면 바로 200
        cached = cache.get(cache_key)
        if cached:
            return Response(cached, status=200)

        # 4) 아직 계산 안 됐으면 큐에 넣고 202 (Accepted)
        enqueue_stock_code(stock_code)
        return Response(
            {"status": "queued", "stock_code": stock_code},
